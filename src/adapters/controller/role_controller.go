...
func (r *RoleController) CreateRole(c *fiber.Ctx) error {
	...
	err := r.roleCrud.CreateRole(c.Context(), &role)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func (r *RoleController) GetAllRoles(c *fiber.Ctx) error {
	roles, err := r.roleCrud.GetAllRoles(c.Context())
	if err != nil {
...
func (r *RoleController) GetRoleByID(c *fiber.Ctx) error {
	id := c.Params("id")
	role, err := r.roleCrud.GetRoleByID(c.Context(), id)
	if err != nil {
		return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func (r *RoleController) UpdateRole(c *fiber.Ctx) error {
	...
	err := r.roleCrud.UpdateRole(c.Context(), id, &role)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": err.Error(),
		})
...
func (r *RoleController) DeleteRole(c *fiber.Ctx) error {
	id := c.Params("id")
	err := r.roleCrud.DeleteRole(c.Context(), id)
	if err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"error": err.Error(),
		})
